...
<head>
    ...
    @endif
    <style>
        .bestiary-nav-letters {
            display: grid;
            grid-template-columns: repeat(var(--bestiary-nav-cols, 17), minmax(0, 1fr));
            gap: 0.25rem;
            width: 100%;
        }
        .bestiary-nav-letters a { text-align: center; padding: 0.2rem 0.35rem; font-size: 0.875rem; line-height: 1.2; border-radius: 0; text-decoration: none; min-width: 0; }
        .bestiary-nav-letters a.is-active { background: oklch(var(--p) / 0.25); color: oklch(var(--pc)); font-weight: 700; }
        .bestiary-nav-letters a:not(.is-active) { color: var(--color-base-content, #fff); opacity: 0.65; }
        .bestiary-nav-letters a.has-creatures:not(.is-active) { font-weight: 700; opacity: 1; color: var(--color-base-content, #fff); }
        .bestiary-nav-letters a:not(.is-active):hover { opacity: 1; background: var(--color-base-200, #2a323c); }
        .bestiary-nav-letters a.has-creatures:not(.is-active):hover { opacity: 1; }
        .bestiary-nav-letters a.is-empty { opacity: 0.45; font-weight: 400; }
...
<body class="min-h-screen bg-base-100 flex flex-col">
    ...
    <main class="flex-1 p-6 max-w-[1344px] w-full mx-auto">
        ...
        <div class="mb-10 space-y-5">
            <div class="flex flex-wrap items-center justify-between gap-4">
                ...
                <div class="flex items-center gap-1 shrink-0">
                    ...
                    <a href="{{ route('bestiary.pdf', $world) }}" class="btn btn-ghost btn-square" title="Выгрузить PDF" aria-label="Выгрузить PDF">
                        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
            ...
            <nav class="bestiary-nav-letters" style="--bestiary-nav-cols: {{ $navColumnCount }};" aria-label="Алфавитный указатель">
                @foreach ($navLetters as $L)
                    @php
                        $cnt = $counts[$L] ?? 0;
                        $isActive = $L === $letter;
                        $classes = 'block w-full ' . ($isActive ? 'is-active' : '') . ($cnt === 0 ? ' is-empty' : '') . ($cnt > 0 ? ' has-creatures' : '');
                        $label = $L === '0-9' ? '1–9' : ($L === \App\Support\BestiaryAlphabet::OTHER_BUCKET ? '…' : $L);
                    @endphp
                    <a href="{{ route('bestiary.index', array_merge(['world' => $world, 'script' => $script, 'letter' => $L], $bestiaryIndexQ)) }}" class="{{ $classes }}">{{ $label }}</a>
                @endforeach
            </nav>
        </div>
        ...
        <div class="text-center mb-8">
            @if (filled($searchQuery))
                <h1 class="text-[2rem] font-semibold text-base-content" style="font-family: 'Cormorant Garamond', Georgia, serif;">Поиск по бестиарию</h1>
            ...
            @else
                ...
                <p class="text-base-content/70 mt-2">
                    @if ($letter === '0-9')
                        Существа из Бестиария с названием на цифру (1–9)
                    @elseif ($letter === \App\Support\BestiaryAlphabet::OTHER_BUCKET)
                        Существа, у которых первая буква названия не относится к выбранному алфавиту (другая раскладка или символ)
                    @else
                        Существа из Бестиария на букву {{ $letter }}
                    @endif
                </p>
            @endif
        </div>
        ...
        @if ($selectedCreatures->isEmpty())
            @if (filled($searchQuery) && $totalCreatures > 0)
                <p class="text-center text-base-content/60 mb-12">Ничего не найдено. Попробуйте другой запрос или <a href="{{ route('bestiary.index', ['world' => $world, 'script' => $script, 'letter' => $letter]) }}" class="link">сбросьте поиск</a>.</p>
            @elseif (filled($searchQuery))
                <p class="text-center text-base-content/60 mb-12">В бестиарии этого мира пока нет существ.</p>
            @else
                <p class="text-center text-base-content/60 mb-12">Нет существ для выбранной буквы.</p>
            @endif
        @else
            <div class="flex flex-wrap gap-4 justify-center mb-12">
                @foreach ($selectedCreatures as $creature)
                    <article class="card bestiary-creature-card bg-base-200 border border-base-300 shadow-none">
                        <figure class="px-0 pt-0 pb-0">
                            @if ($creature->imageUrl())
                                <img src="{{ $creature->imageUrl() }}" alt="{{ $creature->name }}" class="bestiary-creature-img rounded-none">
                            @else
                        ...
                        <div class="card-body px-4 pt-3 pb-4 gap-1 items-stretch text-center">
                            <h2 class="card-title text-lg font-semibold justify-center leading-tight">{{ $creature->name }}</h2>
                            @if (filled($creature->scientific_name))
                                <p class="text-sm text-base-content/50">{{ $creature->scientific_name }}</p>
                            @endif
                            <a href="{{ route('bestiary.creatures.show', [$world, $creature]) }}" class="btn btn-primary btn-sm rounded-none mt-2">Подробнее</a>
                        </div>
                    </article>
                @endforeach
            </div>
        ...
        <div class="border-t border-base-300 pt-6 flex flex-wrap justify-center items-baseline gap-x-6 gap-y-1 text-sm text-base-content/80 @if ($recentCreatures->isEmpty()) mt-8 @endif">
            <span>Всего в Бестиарии: <strong class="text-base-content">{{ $totalCreatures }}</strong></span>
            @if (filled($searchQuery))
                <span>Найдено по запросу: <strong class="text-base-content">{{ $selectedCreatures->count() }}</strong></span>
            @else
                <span>
                    @if ($letter === '0-9')
                        На группу 1–9:
                    ...
                    @endif
                    <strong class="text-base-content">{{ $letterCount }}</strong>
                </span>
            @endif
        </div>
    </main>
        ...
        <div class="bestiary-dialog__viewport">
            ...
            <form method="POST" action="{{ route('bestiary.creatures.store', $world) }}" enctype="multipart/form-data" class="bestiary-dialog__panel" onclick="event.stopPropagation()">
                ...
                <div class="bestiary-dialog__scroll pr-8 pt-2">
                    <h2 id="creature-create-title" class="text-lg font-semibold mb-4 pr-8 border-b border-base-300 pb-2">Новое существо</h2>
