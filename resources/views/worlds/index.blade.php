...
@section('content')
        ...
        <div class="flex flex-col mb-10">
            <x-noema-page-head>
                <x-slot name="title">
                    <h1 class="font-display text-3xl md:text-4xl text-base-content font-semibold tracking-tight">Мои миры</h1>
                </x-slot>
                <x-slot name="center">
                ...
                <x-slot name="actions">
                    <button type="button" class="btn btn-ghost btn-square rounded-none" title="Настройки" aria-label="Настройки" onclick="document.getElementById('worldsListDisplayModal').showModal()">
                        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
                    ...
                    <div class="world-card bg-base-200 border border-base-300 hover:border-primary/30 transition-colors rounded-none overflow-hidden flex flex-col relative">
                        <div class="world-card-inner flex flex-col flex-1 min-h-0">
                            ...
                            <div class="world-card-body flex-1 flex flex-col min-h-0">
                                ...
                                <p class="world-card-synopsis text-sm text-base-content/70">
                                    {{ trim((string) ($world->annotation ?? '')) !== '' ? trim($world->annotation) : 'Здесь могла быть ваша аннотация.' }}
                                </p>
                            </div>
    ...
    <dialog id="worldsListDisplayModal" class="modal modal-middle">
        <div class="modal-box modal-styled noema-settings-modal-box rounded-none">
            <div class="noema-settings-modal-inner">
                <h2 class="font-display text-xl font-semibold text-base-content mb-6">Настройки</h2>
                <form method="POST" action="{{ route('account.worlds-display.update') }}" class="min-h-0">
                @csrf
                @method('PUT')
                <div class="noema-settings-modal-body space-y-4">
                @include('partials.settings-worlds-list-sort-fields', [
                    'currentSort' => $worldsSortCurrent,
                    'selectId' => 'worlds-list-sort',
                    'selectLabel' => 'Последовательность карточек Миров',
                ])
                </div>
    ...
    @if ($errors->has('worlds_list_sort'))
        <script>
            document.addEventListener('DOMContentLoaded', function () {
                document.getElementById('worldsListDisplayModal')?.showModal();
            });
        </script>
    @endif
@endsection
...
@push('styles')
    <style>
        ...
        .modal-box.modal-styled.noema-settings-modal-box {
