...
<head>
    ...
    <title>Фракции — {{ $world->name }} — Noema</title>
...
<body class="min-h-screen bg-base-100 flex flex-col">
    ...
    <main class="flex-1 p-6 max-w-[1344px] w-full mx-auto">
        @php
            $factionIndexQ = filled($searchQuery) ? ['q' => $searchQuery] : [];
        @endphp
        <div class="mb-10">
            <x-noema-page-head>
                <x-slot name="title">
                    <h1 class="text-[1.875rem] font-semibold text-base-content leading-tight min-w-0" style="font-family: 'Cormorant Garamond', Georgia, serif;">Фракции</h1>
                </x-slot>
                <x-slot name="center">
                    <button type="button" class="btn btn-primary btn-sm btn-square shrink-0 rounded-none" id="faction-create-open" title="Новая фракция" aria-label="Новая фракция">
                ...
                <x-slot name="actions">
                    ...
                    <a href="{{ route('factions.index.pdf', $world) }}" class="btn btn-ghost btn-square" title="Выгрузить PDF" aria-label="Выгрузить PDF">
                        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
                    ...
                    @include('partials.activity-log-button', ['world' => $world, 'factionsModuleJournal' => true])
                </x-slot>
            </x-noema-page-head>
            ...
            <div class="mt-[15px] flex flex-wrap items-center gap-3 min-w-0">
                <div class="join rounded-none shrink-0" role="group" aria-label="Набор букв">
                    <a href="{{ route('factions.index', array_merge(['world' => $world, 'script' => \App\Support\BestiaryAlphabet::SCRIPT_CYR], $factionIndexQ)) }}"
                        class="btn btn-sm join-item rounded-none {{ $script === \App\Support\BestiaryAlphabet::SCRIPT_CYR ? 'btn-primary' : 'btn-ghost' }}">А–Я</a>
                    <a href="{{ route('factions.index', array_merge(['world' => $world, 'script' => \App\Support\BestiaryAlphabet::SCRIPT_LAT], $factionIndexQ)) }}"
                        class="btn btn-sm join-item rounded-none {{ $script === \App\Support\BestiaryAlphabet::SCRIPT_LAT ? 'btn-primary' : 'btn-ghost' }}">A–Z</a>
                </div>
                <form method="get" action="{{ route('factions.index', $world) }}" class="flex flex-wrap items-center gap-2 flex-1 min-w-0 max-w-xl">
                    ...
                    @if (filled($searchQuery))
                        <a href="{{ route('factions.index', ['world' => $world, 'script' => $script, 'letter' => $letter]) }}" class="btn btn-sm btn-ghost rounded-none shrink-0">Сбросить</a>
                    @endif
                </form>
            </div>

            <nav class="biography-nav-letters mt-[15px]" style="--biography-nav-cols: {{ $navColumnCount }};" aria-label="Алфавитный указатель">
                @foreach ($navLetters as $L)
                    ...
                    @endphp
                    <a href="{{ route('factions.index', array_merge(['world' => $world, 'script' => $script, 'letter' => $L], $factionIndexQ)) }}" class="{{ $classes }}">{{ $label }}</a>
                @endforeach
            </nav>
        </div>
        ...
        <div class="text-center mb-8">
            @if (filled($searchQuery))
                <h1 class="text-[2rem] font-semibold text-base-content" style="font-family: 'Cormorant Garamond', Georgia, serif;">Поиск по фракциям</h1>
                <p class="text-base-content/70 mt-2">Совпадения по названию во всём модуле (все буквы выбранного алфавита).</p>
            ...
            @else
                ...
                <p class="text-base-content/70 mt-2">
                    @if ($letter === '0-9')
                        Фракции с названием на цифру (1–9)
                    @elseif ($letter === \App\Support\BestiaryAlphabet::OTHER_BUCKET)
                        Фракции, у которых первая буква названия не относится к выбранному алфавиту
                    @else
                        Фракции на букву {{ $letter }}
                    @endif
                </p>
            @endif
        </div>
        ...
        @if ($selectedFactions->isEmpty())
            @if (filled($searchQuery) && $totalFactions > 0)
                <p class="text-center text-base-content/60 mb-12">Ничего не найдено. Попробуйте другой запрос или <a href="{{ route('factions.index', ['world' => $world, 'script' => $script, 'letter' => $letter]) }}" class="link">сбросьте поиск</a>.</p>
            @elseif (filled($searchQuery))
                <p class="text-center text-base-content/60 mb-12">Нет фракций в этом мире.</p>
            @else
                <p class="text-center text-base-content/60 mb-12">Нет фракций для выбранной буквы.</p>
            @endif
        @else
            <div class="flex flex-wrap gap-4 justify-center mb-12">
                @foreach ($selectedFactions as $f)
                    <article class="card biography-card bg-base-200 border border-base-300 shadow-none">
                        ...
                        <div class="card-body px-4 pt-3 pb-4 gap-1 items-stretch text-center">
                            <h2 class="card-title text-lg font-semibold justify-center leading-tight">{{ $f->name }}</h2>
        ...
        @if ($recentFactions->isNotEmpty())
            <div class="border-t border-base-300 pt-8 mt-8">
                @include('factions.partials.recent-factions', ['world' => $world, 'recentFactions' => $recentFactions])
            </div>
        @endif

        <div class="border-t border-base-300 pt-6 flex flex-wrap justify-center items-baseline gap-x-6 gap-y-1 text-sm text-base-content/80 @if ($recentFactions->isEmpty()) mt-8 @endif">
            <span>Всего фракций: <strong class="text-base-content">{{ $totalFactions }}</strong></span>
            @if (filled($searchQuery))
                <span>Найдено по запросу: <strong class="text-base-content">{{ $selectedFactions->count() }}</strong></span>
            @else
                <span>
                    @if ($letter === '0-9')
                        На группу 1–9:
    ...
    <dialog id="faction-create-dialog" class="biography-dialog" aria-labelledby="faction-create-title">
        <div class="biography-dialog__viewport">
            ...
            <form method="POST" action="{{ route('factions.store', $world) }}" enctype="multipart/form-data" class="biography-dialog__panel" onclick="event.stopPropagation()"
                ...
                <h2 id="faction-create-title" class="text-xl font-semibold mb-4 pr-8">Новая фракция</h2>
